...
- ✨ **Interactive Mode** - Space-toggle selection with visual feedback
- 🏗️ **Multi-service builds** - Build one or all services in a single command
- 🎯 **Platform targeting** - Build for different architectures (amd64, arm64, etc.)
- 📦 **Airgap packaging** - Automatically save images as tar files for offline deployment
- 🔄 **Version management** - Automatically reads version from `version.txt`
- 🔗 **Smart SDK sync** - Runs `sync-ark-sdk.sh` if present to sync dependencies
- 🛠️ **Extensible** - Easily add new services to the build system
- ⚡ **Efficient** - Only builds SDK when needed by services
- 💬 **User-friendly** - Interactive prompts for save options and platform selection
...
**Features:**
- ✨ **Space-toggle selection**: Type numbers and press SPACE to toggle services
- 💾 **Save prompts**: Choose whether to save images as tar files
- 🎯 **Platform selection**: Pick your target architecture
- 📦 **Output directory**: Customize where images are saved

**Example Interactive Session:**
```
Select services: 1 3 5
  ✓ Selected: ark-api
  ✓ Selected: ark-cluster-memory  
  ✓ Selected: ark-evaluator
...
| **Visual feedback** | See ✓/− symbols showing what's selected |
...
```
airgap-images/
└── 0.1.39_20250127_143022/      # Version_Timestamp
    ├── manifest.txt              # Build information
    ├── ark-api_0.1.39.tar       # Service image
    ├── ark-api-a2a_0.1.39.tar   # Service image
    └── ...
```
...
```
✗ SDK wheel not found: /path/to/ark_sdk-0.1.39-py3-none-any.whl
