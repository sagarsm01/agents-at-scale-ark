...
helm install mcp-filesystem mcp/filesystem-mcp/chart --set image.repository=filesystem-mcp-server --set image.tag=latest
...
```bash
# View files created by agents using kubectl exec
kubectl exec -it deployment/mcp-filesystem -- ls -la /data/

# Or read specific files
kubectl exec -it deployment/mcp-filesystem -- cat /data/path/to/file.txt
```
...
cd mcp/filesystem-mcp && docker build -t filesystem-mcp-server:latest . && cd ../..
# Load into cluster: kind load docker-image filesystem-mcp-server:latest OR minikube image load filesystem-mcp-server:latest
helm install mcp-filesystem mcp/filesystem-mcp/chart --set image.repository=filesystem-mcp-server --set image.tag=latest
