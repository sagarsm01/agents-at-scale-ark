...
To enable stateful conversations with memory persistence, you must install a Memory resource. The [ARK Cluster Memory Service](/developer-guide/services/ark-cluster-memory) provides a memory implementation with streaming support that can be used with this execution engine.
...
spec:
  ...
  - type: agent
    name: my-agent
  memory:
    name: ark-cluster-memory  # Reference to installed Memory resource
  sessionId: "alice-session"
```

## Next Steps

- [ARK Cluster Memory](/developer-guide/services/ark-cluster-memory) - Memory storage with streaming
